...
export const KEY_SIZE = 50; // 1u = 50px
...
export default class KeyModel {
  ...
  constructor(
    ...
    if (this.location.match(/^e[0-9]+$/i)) {
      ...
      this.pos = locs[0] && locs[0].match(REGEXP_LABEL_POSITION) ? locs[0] : '';
      this.optionLabel =
  ...
  static isDecal(op: KeyOp | null): boolean {
    return !!(op && op.d);
  }

  get isDecal(): boolean {
    return KeyModel.isDecal(this.keyOp);
  }
  ...
  static isEncoder(encoderId: number | null): boolean {
    return encoderId !== null && 0 <= encoderId;
  }
